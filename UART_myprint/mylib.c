...
void integerTransmit(void *p)
{
    ...
    // Se extrag cifrele numarului �n ordine inversa
    do
    {
        // Se converte?te cifra la caracter ASCII
        uint8_t c = x % 10 + '0';
        // Se stocheaza temporar �n buffer auxiliar
        aux2[i] = c;
        i++;
        // Se reduce numarul
        x = x / 10;
    } while (x != 0);
    // Se copiaza cifrele �n ordinea corecta �n bufferul final
    for (int16_t j = i - 1; j >= 0; j--)
    {
        aux1[index] = aux2[j];
        index++;
    }
    // Se adauga caracterele de sf�r?it de linie
    aux1[index] = '\n';
    index++;
    aux1[index] = '\r';
    index++;
...
void hexadecimalTransmit(void *p)
{
    ...
    aux1[index] = 'x';
    index++;
    // Se extrag cifrele hexazecimale �n ordine inversa
    do
    {
    ...
    // Se copiaza cifrele �n ordinea corecta
    for (int16_t j = i - 1; j >= 0; j--)
    {
        aux1[index] = aux2[j];
        index++;
...
void doubleTransmit(void *p)
{
    ...
    else
    {
        ...
        // Se extrag cifrele frac?ionare �n ordine inversa
        while (frac_part != 0)
        {
